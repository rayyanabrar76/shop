  ...
  return (
    ...
    <div className="max-w-3xl mx-auto px-6 pb-10 space-y-6">
      ...
      <div className="bg-(--admin-card) rounded-2xl border border-(--admin-border) overflow-hidden">
        ...
        {showDiscountForm && (
          <div className="p-5 border-b border-(--admin-edge) bg-zinc-50 dark:bg-zinc-800/50 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowDiscountForm(false)} className="px-4 py-2 rounded-xl border border-(--admin-border) text-xs font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800">Cancel</button>
            </div>
          </div>
        )}

        {discounts.length === 0 ? (
          <p className="text-sm text-zinc-500 text-center py-10">No discount codes yet</p>
        ) : (
              ...
              <div key={d.id} className="flex items-center gap-3 px-5 py-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${d.active ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>{d.active ? 'Active' : 'Inactive'}</span>
                  </div>
                ...
                <button onClick={() => toggleDiscount(d.id, d.active)} className="text-xs text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-200 font-medium transition-colors">{d.active ? 'Disable' : 'Enable'}</button>
      ...
      <div className="bg-(--admin-card) rounded-2xl border border-(--admin-border) overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-(--admin-edge)">
          <div className="flex items-center gap-2">
            <HiTruck className="w-4 h-4 text-zinc-500" />
        ...
        {showShippingForm && (
          <div className="p-5 border-b border-(--admin-edge) bg-zinc-50 dark:bg-zinc-800/50 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowShippingForm(false)} className="px-4 py-2 rounded-xl border border-(--admin-border) text-xs font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800">Cancel</button>
            </div>
          </div>
        )}

        {shippingRates.length === 0 ? (
          <p className="text-sm text-zinc-500 text-center py-10">No shipping rates yet, add one above</p>
        ) : (
          <div className="divide-y divide-(--admin-edge)">
            {shippingRates.map(r => (
