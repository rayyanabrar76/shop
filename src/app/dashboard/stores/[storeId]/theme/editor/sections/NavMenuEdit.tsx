      ...
      <div className="p-4 space-y-5">
        ...
        <div>
          ...
          <div className="space-y-2">
            {links.map((link, i) => (
              <div key={i} className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800 rounded-xl px-3 py-2 border border-(--admin-border)">
                ...
                <div className="flex-1 flex flex-col gap-1 min-w-0">
                  <input
                    ...
                    placeholder="Label"
                    className="w-full bg-white dark:bg-zinc-700 border border-(--admin-field-border) rounded-lg px-2 py-1 text-sm outline-none focus:border-(--admin-field-border-focus) text-zinc-900 dark:text-zinc-50"
                  />
                  <input
                    value={link.href}
                    onChange={e => updateLink(i, { href: e.target.value })}
                    placeholder="/page-slug"
                    className="w-full bg-white dark:bg-zinc-700 border border-(--admin-field-border) rounded-lg px-2 py-1 text-xs font-mono outline-none focus:border-(--admin-field-border-focus) text-zinc-500 dark:text-zinc-400"
                  />
                </div>
                <button
            ...
            <button
              onClick={addLink}
              className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl border border-dashed border-(--admin-field-border) text-xs font-semibold text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 hover:border-(--admin-field-border-hover) transition-colors"
            >
