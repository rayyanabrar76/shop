...
export default function AddSectionModal({ onClose, onAdd }: AddSectionModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-(--admin-card) rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-(--admin-border)">

        {/* Header */}
        <div className="px-5 py-4 border-b border-(--admin-edge) flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-50">New Section</h2>
            <p className="text-xs text-zinc-500">Pick a layout and name your section</p>
          </div>
        ...
        <div className="overflow-y-auto p-5 space-y-5 flex-1">
          ...
          <div>
            <label className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 mb-1 block">Section Name</label>
            <input
              autoFocus
              className="w-full rounded-xl border border-(--admin-field-border) px-3 py-2 text-sm outline-none focus:border-(--admin-field-border-focus) transition-all bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-50"
              value={name}
          ...
          <div>
            <label className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 mb-2 block">Layout</label>
                ...
                <button
                  ...
                  className={`group p-3 rounded-xl border-2 transition-all text-left ${
                    layout === l.value
                      ...
                      : 'border-(--admin-border) hover:border-(--admin-field-border-hover)'
                  }`}
                >
                  ...
                  <p className="text-[10px] text-zinc-500 mt-0.5">{l.description}</p>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* Footer */}
        <div className="px-5 py-4 border-t border-(--admin-edge) flex items-center justify-end gap-2">
          <button
            onClick={onClose}
