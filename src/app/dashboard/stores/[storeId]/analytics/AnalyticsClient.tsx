    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center gap-3">
          ...
          <BarChart2 size={16} className="text-zinc-400 dark:text-zinc-500" />
          <span className="font-semibold text-sm text-zinc-800 dark:text-zinc-100">Analytics</span>
          <span className="text-xs text-zinc-400 dark:text-zinc-500 ml-1">Last 30 days</span>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map(card => (
            <div key={card.label} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-5">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">Daily breakdown · last 30 days</p>
            </div>
          ...
          <div className="flex justify-between mt-2">
            {revenueChart.filter((_, i) => i % 5 === 0 || i === revenueChart.length - 1).map(d => (
              <span key={d.date} className="text-[10px] text-zinc-400 dark:text-zinc-500">{fmtDate(d.date)}</span>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-6">
            ...
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-5">Breakdown of all orders</p>
            <div className="space-y-3">
                ...
                return (
                  <div key={status}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-zinc-400 dark:text-zinc-500">{count} ({pct}%)</span>
                    </div>
              ...
              {stats.totalOrders === 0 && <p className="text-sm text-zinc-400 dark:text-zinc-500 text-center py-4">No orders yet</p>}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-6">
            ...
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-5">By units sold</p>
            {topProducts.length === 0 ? (
              <div className="text-center py-8">
                <Package size={28} className="mx-auto text-zinc-200 dark:text-zinc-700 mb-2" />
                <p className="text-sm text-zinc-400 dark:text-zinc-500">No sales yet</p>
              </div>
            ) : (
              <div className="space-y-3">
                {topProducts.map((p, i) => {
                    ...
                    <div key={p.id}>
                      <div className="flex items-center justify-between mb-1 gap-2">
                        ...
                        <div className="shrink-0 text-right">
                          ...
                          <span className="text-[10px] text-zinc-400 dark:text-zinc-500 ml-1">· {fmt(p.revenue)}</span>
                        </div>
                      </div>
                      <div className="h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-6">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-5">Each bar = one day</p>
          <BarChart data={revenueChart} valueKey="revenue" color="#10b981" />
          <div className="flex justify-between mt-2">
            <span className="text-[10px] text-zinc-400 dark:text-zinc-500">{fmtDate(revenueChart[0]?.date ?? '')}</span>
            <span className="text-[10px] text-zinc-400 dark:text-zinc-500">{fmtDate(revenueChart[revenueChart.length - 1]?.date ?? '')}</span>
          </div>
        </div>
      </div>
    </div>
