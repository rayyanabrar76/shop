...
export default function LoginForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/storefront/${subdomain}/auth/login`, {
        method: 'POST',
      ...
      router.push(redirectTo ?? `/store/${subdomain}`)
      router.refresh()
    } catch {
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <p className="text-sm opacity-60 mb-8">Welcome back to the store.</p>
      ...
      <div className="mt-6 text-center space-y-2">
        <Link
          ...
          className="text-xs opacity-50 hover:opacity-100 transition-opacity block"
        >
          Forgot your password?
        </Link>
        <p className="text-sm opacity-60">
          Don&apos;t have an account?{' '}
          <Link href={`/store/${subdomain}/signup`} className="font-bold opacity-100 hover:underline" style={{ color: primaryColor }}>
            Sign up
          </Link>
        </p>
      </div>
